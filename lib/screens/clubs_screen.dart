...
class _ClubsScreenState extends State<ClubsScreen> {
  ...
  Future<_ClubsData> _load() async {
    ...
    await Entitlements.refresh(); // so club-creation gating knows the tier
  ...
  Future<void> _create() async {
    final uid = _uid;
    if (uid == null) return;
    // Free tier can create one (public) club; Premium is unlimited.
    final limit = Entitlements.createdClubLimit;
    if (limit != null) {
      final owned = await _client.from('clubs').select('id').eq('owner_id', uid);
      if (owned.length >= limit && mounted) {
        await showUpgradeSheet(
          context,
          title: 'You\'ve reached the free club limit',
          message: 'Free members can create one public club. Upgrade to Premium '
              'for unlimited clubs — including private ones.',
        );
        return;
      }
    }
